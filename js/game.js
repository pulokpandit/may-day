...
//    buttonStartIns.cursor = "pointer";
//    buttonStartIns.addEventListener("click", function (evt) {
//        playSound('soundClick');
//        goPage('select');
//    });

    buttonStartIns.cursor = "pointer";
    buttonStartIns.addEventListener("mousedown", function (evt) {
        var result = BackEndService.gameStart();
    ...
    buttonFullscreenoff.cursor = "pointer";
    buttonFullscreenoff.addEventListener("click", function (evt) {
    ...
    buttonExit1.cursor = "pointer";
    buttonExit1.addEventListener("click", function (evt) {
        if (!isPanelShowing) {
            window.location.href = "../../game_redirect.php?type=gamecenter";
        }
    });
...
//                    TweenMax.killTweensOf(curTimerTxt);
    ...
    if (!$.editor.enable) {
        ...
        TweenMax.to(gameData, 2, {overwrite: true, onComplete: function () {
                ...
                result.then(function (data) {
                    if (data.error_url == "")
                    {
//                        if (BackEndService.GAstatus == 1)
//                        {
//                            AnalyticsResource_Slot("Source", "points", playerData.credit);
//                            if (isWin)
//                            {
//                                Analyticsprogression_Slot("Complete", "Fruit Loose");
//                            } else
//                            {
//                                Analyticsprogression_Slot("Fail ", "Fruit Loose");
//                            }
//                        }
                        if (data.game_end == "YES") {
                            if (data.end_result == "LOSS") {
                                isWin = false;
                            }
                            if (data.end_result == "WIN") {
                                isWin = true;
                            }
...
//    newTxt.stuckTimer = 100;
    ...
    //newTxt.drawingStroke = drawingStroke;// code edit by pulok
...
function toggleFullScreen(isfullScreen) {
    buttonFullscreen.visible = !isfullScreen;
    buttonFullscreenoff.visible = isfullScreen;
...
function pulseAnimation(button) {
    createjs.Tween.get(button, {loop: -1})
